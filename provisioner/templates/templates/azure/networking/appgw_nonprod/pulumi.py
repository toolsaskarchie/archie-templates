...
class AzureAppGatewayNonProdTemplate(InfrastructureTemplate):
    ...
    def create_infrastructure(self):
        ...
        cloud_init_script = (
            ...
            "<!DOCTYPE html><html><head><title>Archie - App Gateway</title>\n"
            "<style>body{font-family:-apple-system,sans-serif;background:#0a0b0f;color:#e4e5e9;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0}"
            ".c{background:#12141a;border:1px solid #1e2130;border-radius:16px;padding:48px;max-width:520px;text-align:center}"
            ".l{width:48px;height:48px;background:#4f8ff7;border-radius:12px;display:flex;align-items:center;justify-content:center;margin:0 auto 24px;font-weight:bold;color:#fff;font-size:20px}"
            "h1{font-size:24px;margin:0 0 8px}.s{color:#8b8fa3;font-size:14px;margin-bottom:24px}"
            ".i{background:#1a1d25;border-radius:8px;padding:16px;text-align:left;font-size:13px}"
            ".r{display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #1e2130}.r:last-child{border:none}"
            ".k{color:#8b8fa3}.v{color:#4f8ff7;font-family:monospace}"
            ".b{display:inline-block;padding:4px 12px;border-radius:20px;background:#34d39912;color:#34d399;font-size:11px;font-weight:600;margin-top:16px}</style></head><body>\n"
            '<div class="c"><div class="l">A</div>\n'
            "<h1>Load Balancing Active</h1>\n"
            '<p class="s">Azure Application Gateway distributing traffic across backend VMs</p>\n'
            f'<div class="i">'
            f'<div class="r"><span class="k">Project</span><span class="v">{project}</span></div>'
            f'<div class="r"><span class="k">Environment</span><span class="v">{env}</span></div>'
            '<div class="r"><span class="k">Backend VM</span><span class="v">$H</span></div>'
            f'<div class="r"><span class="k">Region</span><span class="v">{location}</span></div>'
            f'<div class="r"><span class="k">VM Size</span><span class="v">{vm_size}</span></div>'
            '<div class="r"><span class="k">Managed By</span><span class="v">Archie IDP</span></div>'
            "</div>\n"
            '<span class="b">Deployed via AskArchie</span>\n'
            "</div></body></html>\n"
            "EOF\n"
