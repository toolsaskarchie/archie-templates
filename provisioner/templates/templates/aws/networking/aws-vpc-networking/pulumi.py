...
class AwsVpcNetworking(InfrastructureTemplate):
    ...
    def cfg(self, key, default=None):
        """Helper to check both self.config and self.config.parameters for a key"""
        if hasattr(self.config, 'get') and key in self.config:
            return self.config.get(key, default)
        if hasattr(self.config, 'parameters') and hasattr(self.config.parameters, 'get') and key in self.config.parameters:
            return self.config.parameters.get(key, default)
        return default

    def create_infrastructure(self):
        project = self.cfg('project_name', 'proj')
        env = self.cfg('environment', 'prod')
        region = self.cfg('region', 'us-east-1')
        ...
        self.vpc = factory.create('aws:ec2:Vpc', f'vpc-{project}-{env}',
            cidr_block=self.cfg('vpc_cidr', '10.111.0.0/16'),
        ...
        self.public_subnet = factory.create('aws:ec2:Subnet', f'public-subnet-{project}-{env}',
            vpc_id=self.vpc.id,
            cidr_block=self.cfg('public_subnet_cidr', '10.111.1.0/24'),
            availability_zone=self.cfg('availability_zone', 'us-east-1a'),
        ...
        self.private_subnet = factory.create('aws:ec2:Subnet', f'private-subnet-{project}-{env}',
            vpc_id=self.vpc.id,
            cidr_block=self.cfg('private_subnet_cidr', '10.111.2.0/24'),
            availability_zone=self.cfg('availability_zone', 'us-east-1a'),
