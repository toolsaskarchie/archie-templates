# ─── Test Panel: AWS Lambda + Function URL (TF) ───────────────────────────────
# Reference blueprint for the test-panel pattern. Every other blueprint in
# templates/test-panel/ mirrors this structure.
#
# Resources per profile:
#   Non-prod:  7  (log group, role, basic-attach, archive, lambda, url,
#                  url-perm × 2 minus one when prod-only)
#   Prod:      9  (+ X-Ray attach + SQS DLQ + DLQ policy = +2 over nonprod)
#
# Drift-injectable fields (validated to surface as drift + remediate cleanly):
#   | Field              | Inject CLI                                                                      | Verify CLI                                                            |
#   |--------------------|---------------------------------------------------------------------------------|-----------------------------------------------------------------------|
#   | memory_size        | aws lambda update-function-configuration --function-name FN --memory-size 512   | aws lambda get-function-configuration --function-name FN --query MemorySize |
#   | timeout            | aws lambda update-function-configuration --function-name FN --timeout 30        | aws lambda get-function-configuration --function-name FN --query Timeout    |
#   | retention_in_days  | aws logs put-retention-policy --log-group-name /aws/lambda/FN --retention 14    | aws logs describe-log-groups --query "logGroups[?logGroupName=='X']"        |
#   | env PAGE_TITLE     | aws lambda update-function-configuration --function-name FN --environment 'Variables={PAGE_TITLE=Drifted}' | aws lambda get-function-configuration ... |
#
# Fields that produce drift NOISE (do NOT inject — known false positives):
#   - managedPolicyArns on role  (populated by aws_iam_role_policy_attachment)
